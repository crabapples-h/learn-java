<template>
	<ul className="mui-table-view" style="background-color: #fff;">
		...
		<router-link :to=`/hospital/info/${item.id}` tag="span" :key="item.id" v-for="item in dataList">
...
<script>
	module.exports = {
		...
		activated(){
			this.spinner = true
			const _this = this
			let param = JSON.parse(localStorage.getItem('hospitalQuery'))
			let url = UTIL.url('member/queryHealthMed4App.do')
