<template>
	<ul className="mui-table-view" style="background-color: #fff;">
		...
		<router-link :to=`/work/info/${item.id}` tag="span" :key="item.id" v-for="item in dataList">
...
<script>
	module.exports = {
		...
		activated(){
			this.spinner = true
			const _this = this
			let param = JSON.parse(localStorage.getItem('workQuery'))
			let url = UTIL.url('/member/queryEmploy4App.do')
