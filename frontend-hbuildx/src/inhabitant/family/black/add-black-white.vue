<template>
	<div class="option" style="margin-top: 6rem;">
		<select v-model="reason" v-if="familyInfo.blacklist==='否'">
...
<script>
	module.exports = {
		data() {
			return {
				familyInfo: JSON.parse(localStorage.getItem('familyData-full')),
				reason: '',
				remark: '',
			}
		},
		activated() {
			axios.get(UTIL.url(`getPeoInfoById.do?id=${this.$route.params.id}`)).then(response => {
				if (response.data.state === 1) {
					...
					console.log(response.data.content.blacklist)
				}
			})
		},
		methods: {
			...
			submit() {
				let data = {
					id: this.familyInfo.id,
					bl: this.familyInfo.blacklist === '否' ? '是' : '否',
					remark:  this.remark,
					reason: this.reason
				}
				...
				axios.post(url1,data).then(response=>{
					if(response.data.state===1){
						alert('操作成功')
						this.$router.push(`/family/info/${this.familyInfo.id}`)
					}
				})
			}
		}
