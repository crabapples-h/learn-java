...
////
////                List<ResultMapping> resultMappings = new ArrayList<>(resultMap.getResultMappings());
////
////                for (TableFieldInfo tableFieldInfo : fieldInfoList) {
////                    if (!tableFieldInfo.getColumn().equals("role_list")) {
////                        ResultMapping mapping = new ResultMapping.Builder(config, tableFieldInfo.getProperty(),
////                                tableFieldInfo.getColumn(), tableFieldInfo.getPropertyType()).build();
////                        resultMappings.add(mapping);
////                    }
////                }
////
//////                // a. 首先，为集合内的元素（String）创建一个简单的 ResultMapping。
////                ResultMapping roleItemMapping = new ResultMapping.Builder(config, "value", "role_list", String.class).build();
//////                // b. 然后，创建一个临时的、只包含这个 String 映射的 ResultMap。描述了如何从一行数据中构建一个 String。
////                ResultMap stringResultMap = new ResultMap.Builder(config, "stringResultMap_temp", String.class,
////                        Collections.singletonList(roleItemMapping)).build();
////
////                config.addResultMap(stringResultMap);
////
//////                // c. 最后，创建 <collection> 对应的 ResultMapping。
////                ResultMapping roleListMapping = new ResultMapping.Builder(config, "roleList") // 目标属性是 roleList
////                        .javaType(List.class) // 它的 Java 类型是 List
////                        .flags(Collections.singletonList(ResultFlag.ID))
////                        // 告诉 MyBatis，List 中的每个元素都应该按照 stringResultMap 的规则来构建
////                        .nestedResultMapId(stringResultMap.getId())
////                        .build();
////
////
//////                newMappings.add(mapping);
//////                resultMappings.add(idMapping);
////                resultMappings.add(roleListMapping);
////
////                ResultMap newResultMap = new ResultMap.Builder(config, "custom_result_map", tableClass, resultMappings).build();
//////                config.addResultMap(newResultMap);
////
////                Field fieldResultMaps = MappedStatement.class.getDeclaredField("resultMaps");
////                fieldResultMaps.setAccessible(true);
////                fieldResultMaps.set(mappedStatement, Collections.singletonList(newResultMap));
//            }
//        }
