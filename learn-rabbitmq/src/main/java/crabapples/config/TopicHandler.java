...
public class TopicHandler {
    ...
    @RabbitListener(queues = TopicConfigure.QUEUE_NAME)
    @RabbitHandler
    public void messageHandler(Channel channel, Message message) throws IOException {
        String messageStr = new String(message.getBody(), StandardCharsets.UTF_8);
        log.info("收到MQ消息,监听器:[{}],队列:[{}],内容:[{}]", "messageHandler", TopicConfigure.QUEUE_NAME, messageStr);
     ...
     */
    @RabbitListener(queues = TopicConfigure.QUEUE_NAME2,concurrency = "20")
    @RabbitHandler
    public void messageHandler2(Channel channel, Message message) throws IOException {
        String messageStr = new String(message.getBody(), StandardCharsets.UTF_8);
        log.info("收到MQ消息,监听器:[{}],队列:[{}],内容:[{}]", "messageHandler2", TopicConfigure.QUEUE_NAME2, messageStr);
