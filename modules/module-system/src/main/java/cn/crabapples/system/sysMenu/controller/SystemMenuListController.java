...
@Tag(name = "系统接口[菜单]")
@Slf4j
@RequestMapping("/api/system/menu/list")
public class SystemMenuListController extends BaseController {

    private final SystemMenusService menusService;
    private final SystemRoleMenusService roleMenusService;

    public SystemMenuListController(SystemMenusService menusService, SystemRoleMenusService roleMenusService) {
     ...
     */
    @GetMapping("/user")
    @Operation(summary = "获取当前用户菜单列表", description = "获取当前用户菜单列表接口")
    public ResponseDTO<List<SysMenu>> getUserMenus() {
        log.info("收到请求->获取[当前用户]菜单列表");
        List<SysMenu> menus = menusService.getUserMenusList();
        log.debug("返回结果->获取[当前用户]菜单列表:[{}]", menus);
     ...
     */
    @GetMapping("/page")
    @Operation(summary = "获取分页菜单列表", description = "获取所有分页列表接口")
    public ResponseDTO<Page<SysMenu>> getMenuPage(@RequestParam(required = false, defaultValue = "1") Integer pageIndex,
        ...
        Page<SysMenu> list = menusService.getMenuListPage(pageIndex, pageSize, form);
        log.debug("返回结果->获取[分页]菜单列表成功:[{}]", list);
